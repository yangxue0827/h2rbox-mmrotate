...
def rotate_crop(img, theta=0, size=(768, 768), gt_bboxes=None):
    device = img.device
    n, c, h, w = img.shape
    size_h, size_w = size
    crop_h = (h - size_h) // 2
...
class WSL(RotatedSingleStageDetector):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(WSL, self).__init__(backbone, neck, bbox_head, train_cfg,
    ...
    def forward_train(self,
        ...
        img1, gt_bboxes = rotate_crop(img, 0, self.crop_size, gt_bboxes)
        x = self.extract_feat(img1)
        img2 = rotate_crop(img, rot, self.crop_size)
